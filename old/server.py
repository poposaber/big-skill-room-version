...
PLAYING_GAME_COUNT = "playing_game_count"
...
class Server(Interactable):
    def __init__(self):
        ...
        self.user_socket_dict = {} # user_socket: {USERNAME: <username>, IS_WAITING: bool, IS_PLAYING: bool, }
        self.game_server_socket_dict = {} # game_server_socket: {}
    ...
    def help_login(self, client_socket: socket.socket, msg: str):
        ...
        with LOCK:
            ...
            elif username in [self.user_socket_dict[sock][USERNAME] for sock in self.user_socket_dict.keys()]:
                self.send_message_format_args(client_socket, Protocols.Response.LOGIN_RESULT, -2)
            else:
                self.user_socket_dict[client_socket][USERNAME] = username
    ...
    def record_win(self, client_socket: socket.socket):
        self.user_db[self.user_socket_dict[client_socket][USERNAME]]["games_won"] += 1
        self.save_user_db()
    ...
    def send_status(self, client_socket: socket.socket):
        games_played = self.user_db[self.user_socket_dict[client_socket][USERNAME]]["games_played"]
        games_won = self.user_db[self.user_socket_dict[client_socket][USERNAME]]["games_won"]
    ...
    def handle_client(self, client_socket: socket.socket, addr: tuple[str, int]):
        try:
            role, = self.receive_message_format_and_parse(client_socket, Protocols.Command.ROLE)
            if role == "user":
    ...
    def handle_user(self, user_socket: socket.socket, addr: tuple[str, int]):
        self.user_socket_dict[user_socket] = {USERNAME: None, IS_WAITING: False, IS_PLAYING: False}
        self.active_connections += 1
        print(f"[NEW CONNECTION] {addr} connected.")
        self.send_message_format(user_socket, Protocols.Response.WELCOME)
            ...
            try:
                ...
                match command:
                    ...
                    case Protocols.Command.EXIT.name:
                        self.send_message_format(user_socket, Protocols.Response.GOODBYE)
                        #user_socket.send(Protocols.Response.GOODBYE.encode())
                        break
                    case Protocols.Command.RECORD_WIN.name:
                        self.record_win(user_socket)
                        #user_socket.send(Protocols.Response.WIN_RECORD_DONE.encode())
                    ...
                    case _:
                        self.send_message_format(user_socket, Protocols.Response.UNKNOWN_COMMAND)
                        #user_socket.send(Protocols.Response.UNKNOWN_COMMAND.encode())
            except Exception as e:
                print(f"[EXCEPTION] {e}")
                break
        
        self.user_socket_dict.pop(user_socket)
        self.active_connections -= 1
